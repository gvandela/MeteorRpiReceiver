...
'''
scheduler_v0.3.0 is a copy of v0.2.9:
    Added pseudo color processing of channel 3, 5 & 6, whichever is available
...
# decodes the .s file that was generated in the last 30 minutes,
# removes older .s files, returns the BMP file name
def decode_capture():
...
def process_image(image_1, image_2):
    if image_1 != 'none':
        ...
        if (sum(active_channels) != 3):
            logging.debug('Amount of active channels incorrect: channels ' + str(active_channels))
        ...
        if (active_channels[:2] == [1, 1]) and (ignore_evening_passes is False):
        ...
        if (active_channels == [1, 1, 1, 0, 0, 0]) and (ignore_evening_passes is False):
            image_dir = os.path.join(main_folder, 'Images', image_1 + '_123.jpg')
            image = Image.merge("RGB", (ch_2, ch_1, ch_0))
            image.save(image_dir, quality=90, subsampling=0)
            list_of_images.append(image_dir)

        if (active_channels == [1, 1, 0, 1, 0, 0]) and (ignore_evening_passes is False):
        ...
        # Pseudo color images
        # Water ice seems to reflect in bands 1 & 2, but not in band 3
        # None of these bands are visible at night (especially in winter time)
        if (active_channels == [1, 1, 1, 0, 0, 0]) and (ignore_evening_passes is False):
            # combine useful channels
            image_dir = os.path.join(main_folder, 'Images', image_1 + '_123_pseudo.bmp')
            image = Image.merge("RGB", (ch_2, ch_1, ch_0))
            image.save(image_dir)

            temp_im_dir = os.path.join(main_folder, 'Images', image_1 + '_temp.bmp')
            pseudo_color(image_dir, temp_im_dir, -1, 1, 1)  # G+B-R

            image_pseudo = Image.open(temp_im_dir)
            image_dir = os.path.join(main_folder, 'Images', image_1 + '_ice.jpg')
            image_pseudo.save(image_dir, quality=90, subsampling=0)
            list_of_images.append(image_dir)

        # Thermal infrared band 5
        if (active_channels[4] == 1):
            # combine useful channels
            image_dir = os.path.join(main_folder, 'Images', image_1 + '_555_pseudo.bmp')
            image = Image.merge("RGB", (ch_4, ch_4, ch_4))
            image = ImageOps.autocontrast(image)
            image.save(image_dir)

            temp_im_dir = os.path.join(main_folder, 'Images', image_1 + '_temp.bmp')
            pseudo_color(image_dir, temp_im_dir, 1, 0, 0)  # G

            image_pseudo = Image.open(temp_im_dir)
            image_dir = os.path.join(main_folder, 'Images', image_1 + '_thermal.jpg')
            image_pseudo.save(image_dir, quality=90, subsampling=0)
            list_of_images.append(image_dir)
...
def pseudo_color(input, output, rCh, bCh, gCh):
    # launch c program to apply pseudo color to a weighted combination of RGB
    if platform.machine() == 'x86_64':
        command = './Pseudo_color/pseudo ' + input + " " + output + " " + \
            str(rCh) + " " + str(gCh) + " " + str(bCh)
    elif platform.machine().startswith('arm'):
        command = './Pseudo_color/pseudo_arm ' + input + " " + output + " " + \
            str(rCh) + " " + str(gCh) + " " + str(bCh)
    else:
        logging.error('platform unsupported by pseudo color converter')
        return -1		# I'm not sure what else to do here...
    logging.debug('running: ' + command)
    pseudo_color_process = subprocess.Popen([command], shell=True)
    return_code = pseudo_color_process.wait()
    logging.debug('pseudo_color ran with returncode: %s' % return_code)
...
while True:
    ...
    # 2a) wait for positive elevation
    ...
    logging.debug('started ' + grc_script + ' with PID:' + str(grc_process.pid)\
        + ' ' + str(SDR_capture)+ ' started at ' + str(datetime.datetime.now()))
